import { C64_TOKENS, VIC20_BASIC_START_ADDR } from './compiler';

// Reuse the same interfaces (they're compatible)
...
export class VIC20BasicTokenizer {
    ...
    generatePrg(program: C64BasicProgram): Uint8Array {
        ...
        for (let i = 0; i < program.lines.length; i++) {
            ...
            if (i === 0) {
                console.log(`  🎯 First line structure: nextAddr=0x${nextAddr.toString(16)}, lineNum=${line.lineNumber}, code starts with 0x${line.code[0]?.toString(16) || '??'}`);
            }
    ...
    compile(source: string, startAddr?: number): Uint8Array {
        ...
        const processedLines: { lineNumber: number; code: string; sourceLine: string; hasLabel?: boolean; labelName?: string; sourcePosition: number }[] = [];
        const unnumberedLabels: { label: string; sourceLine: string; position: number }[] = []; // Labels without line numbers
        ...
        const seenLineNumbers = new Map<number, { lineNumber: number; code: string; sourceLine: string; sourcePosition?: number }>();
        for (const line of processedLines) {
            ...
            if (!existing) {
                ...
                if (isRem && !existingIsRem) {
                    ...
                    seenLineNumbers.set(newLineNumber, { ...existing, lineNumber: newLineNumber });
                    seenLineNumbers.set(line.lineNumber, line);
