...
export class VIC20BasicTokenizer {
    ...
    compile(source: string, startAddr?: number): Uint8Array {
        ...
        const pendingLabels: { label: string }[] = [];
        ...
        const seenLineNumbers = new Map<number, { lineNumber: number; code: string; sourceLine: string }>();
        for (const line of processedLines) {
            ...
            if (!existing) {
                ...
                if (isRem && !existingIsRem) {
                    ...
                    if (remLineNumber < 0) remLineNumber = 0; // VIC-20 allows line numbers 0-9
                    ...
                    if (remLineNumber < line.lineNumber && remLineNumber >= 0) {
                        seenLineNumbers.set(remLineNumber, { ...line, lineNumber: remLineNumber });
                        ...
                        seenLineNumbers.set(nextLineNumber, { ...line, lineNumber: nextLineNumber });
