...
function setupDebugControls() {
  ...
  $("#apiLoginButton").click(_loginUser);
  $("#apiRegisterButton").click(_registerUser);
...
function showInstructions() {
  ...
  if (vcanvas) {
    vcanvas.on('focus', () => {
      ...
      if (platform && platform.isRunning && platform.isRunning()) {
        div.fadeIn(200);
  ...
  (window as any).IDE = {
    ...
    getApple2API: () => {
      ...
      if (platform_id !== 'apple2e') {
        console.error('❌ Not on Apple IIe platform. Current platform:', platform_id);
        return null;
      }
      if (typeof (platform as any).getApple2API !== 'function') {
        console.error('❌ Platform does not have getApple2API method');
        return null;
      }
      const api = (platform as any).getApple2API();
      if (!api) {
    ...
    breakApple2: () => {
      const api = (window as any).IDE.getApple2API();
      if (!api) {
