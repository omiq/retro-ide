...
keyboard = new Keyboard({
    ...
    inputEnabledFunction: () => {
        // When embedded, allow keyboard input when the screen is focused
        if (parsedQuery.embed !== undefined) {
            return document.activeElement && (
                document.activeElement.id === "screen" || 
                document.activeElement.id === "paste-text" ||
                document.activeElement.tagName === "CANVAS"
            );
        }
        // Original behavior for non-embedded mode
        return document.activeElement && document.activeElement.id === "paste-text";
    },
    keyLayout,
    dbgr,
});
...
(function () {
    ...
    // Hide visual elements when embedded
    if (parsedQuery.embed !== undefined) {
        $("#header-bar").hide();
