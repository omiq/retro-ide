...
exports.VIC20ChipsMachine = void 0;
    ...
    window.vic20_debug = {
        ...
        openIframeWithCurrentProgram: () => {
            ...
            if (output && output instanceof Uint8Array) {
                const url = window.vic20_debug.generateIframeURL(output);
...
class VIC20ChipsMachine {
    constructor() {
        this.module = null;
        this.canvas = null;
        this.running = false;
        this.programLoaded = false; // Track if a program has been loaded
        this.isLoadingProgram = false; // Prevent infinite loops during loading
        this.emulatorFocused = false; // Track if emulator is focused
    ...
    enableJoystickSupport() {
        ...
        if (window.h && typeof window.h.__sargs_add_kvp === 'function') {
            try {
                window.h.__sargs_add_kvp('joystick', 'true');
    ...
    async waitForModuleDetection() {
        // Wait for module to be detected
        let attempts = 0;
        const maxAttempts = 20; // 2 seconds
        while (attempts < maxAttempts) {
            if (this.module || window.Module || window.h) {
                this.detectModule();
                break;
            }
    ...
    run() {
        ...
        if (this.module && typeof this.module.vic20_run === 'function') {
            this.module.vic20_run();
        }
        else if (typeof window.vic20_run === 'function') {
            window.vic20_run();
        }
    }
    stop() {
    ...
    reset() {
        if (this.module && typeof this.module.vic20_reset === 'function') {
            this.module.vic20_reset();
        }
        else if (typeof window.vic20_reset === 'function') {
            window.vic20_reset();
        }
    }
    advanceFrame(trap) {
    ...
    executeLoadedProgram() {
        ...
        const executionFunctions = Object.keys(h).filter(fn => fn.toLowerCase().includes('run') ||
    ...
    loadProgramViaURL(program) {
        ...
        if (window.h) {
            ...
            const urlFunctions = Object.keys(h).filter(fn => fn.toLowerCase().includes('url') ||
    ...
    loadProgramViaDrop(prgData) {
        ...
        const h = window.h;
        const availableWindowFunctions = Object.keys(window).filter(key => typeof window[key] === 'function' &&
            (key.toLowerCase().includes('drop') || key.toLowerCase().includes('load') || key.toLowerCase().includes('run')));
        const availableModuleFunctions = this.module ? Object.keys(this.module).filter(key => typeof this.module[key] === 'function' &&
            (key.toLowerCase().includes('drop') || key.toLowerCase().includes('load') || key.toLowerCase().includes('run'))) : [];
    ...
    debugCompilationOutput() {
        ...
        if (typeof window.worker !== 'undefined') {
            ...
            if (window.worker.store) {
                ...
                if (window.worker.store.workfs['main']) {
                    ...
                    console.log("Main file first 32 bytes:", Array.from(mainFile.data.slice(0, 32)).map((b) => b.toString(16).padStart(2, '0')).join(' '));
                }
                // Check for map file
                if (window.worker.store.workfs['main.map']) {
                    const mapFile = window.worker.store.workfs['main.map'];
        ...
        if (typeof window.builder !== 'undefined') {
            ...
            console.log("Builder steps:", window.builder.steps);
        }
        // Check for any global compilation state
        if (typeof window.current_output !== 'undefined') {
            console.log("✅ Current output found:", window.current_output);
            console.log("Current output length:", (_a = window.current_output) === null || _a === void 0 ? void 0 : _a.length);
            // Log the first 32 bytes of the current output
            if (window.current_output && window.current_output.length > 0) {
                console.log("Current output first 32 bytes (hex):", Array.from(window.current_output.slice(0, 32)).map((b) => b.toString(16).padStart(2, '0')).join(' '));
                console.log("Current output first 32 bytes (decimal):", Array.from(window.current_output.slice(0, 32)).join(' '));
            }
        }
        // Check for compilation parameters
        if (typeof window.compparams !== 'undefined') {
            console.log("✅ Compilation parameters found:", window.compparams);
        }
        // Check for debug symbols
        if (typeof window.platform !== 'undefined' && window.platform.debugSymbols) {
            console.log("✅ Debug symbols found:", window.platform.debugSymbols);
        }
    }
    addGlobalDebugFunctions() {
        // Add global functions for debugging and testing
        window.vic20_debug = {
            loadProgram: (data) => this.loadProgram(data),
            getCanvas: () => this.getCanvas(),
            getModule: () => this.module,
            getCPUState: () => this.getCPUState(),
            readMemory: (address) => this.read(address),
            writeMemory: (address, value) => this.write(address, value),
            reset: () => this.reset(),
            run: () => this.run(),
            stop: () => this.stop(),
    ...
    tryDirectMemoryLoading(prgData) {
    ...
    destroy() {
        ...
        if (window.vic20Instance === this) {
            window.vic20Instance = null;
        }
    ...
    setKeyInput(key, code, flags) {
        ...
        else if (window.h && typeof window.h.vic20_joystick === 'function') {
            window.h.vic20_joystick(this.joymask0, this.joymask1);
        }
        else if (window.h && typeof window.h.__sargs_add_kvp === 'function') {
            // Try to set joystick state via URL parameters
            try {
                window.h.__sargs_add_kvp('joystick0', this.joymask0.toString());
                window.h.__sargs_add_kvp('joystick1', this.joymask1.toString());
            }
            catch (e) {
    ...
    simulateKeyboardInput(text) {
        ...
        const h = window.h;
        if (h) {
            const keyboardFunctions = Object.keys(h).filter(fn => fn.toLowerCase().includes('key') ||
    ...
    verifyProgramLoaded(expectedData) {
        ...
        const executionFunctions = availableFunctions.filter(func => func.toLowerCase().includes('run') ||
    ...
    verifyAndExecuteLoadedROM(prgData) {
    ...
    callDropFunctionsWithoutMemory(prgData) {
    ...
    simulateFileReading(prgData) {
        ...
        let heapU8 = null;
        const h = (_a = this.module) === null || _a === void 0 ? void 0 : _a.h;
        // Try multiple ways to find HEAPU8
        if (window.t && window.t instanceof Uint8Array) {
            heapU8 = window.t;
            console.log("✅ Found HEAPU8 via window.t");
        }
        else if (window.r && window.r instanceof Uint8Array) {
            heapU8 = window.r;
            console.log("✅ Found HEAPU8 via window.r");
        }
        else if (h === null || h === void 0 ? void 0 : h.HEAPU8) {
            heapU8 = h.HEAPU8;
            console.log("✅ Found HEAPU8 via h.HEAPU8");
        }
        else if (h === null || h === void 0 ? void 0 : h.HEAP8) {
            heapU8 = new Uint8Array(h.HEAP8.buffer);
            console.log("✅ Found HEAPU8 via h.HEAP8 conversion");
        }
        else if (h === null || h === void 0 ? void 0 : h.HEAP) {
        ...
        try {
            ...
            for (const addr of addressesToCheck) {
                try {
                    ...
                    if (window.t && typeof window.t[addr] !== 'undefined') {
                        value = window.t[addr];
                        console.log(`  - 0x${addr.toString(16).padStart(4, '0')}: 0x${value.toString(16).padStart(2, '0')} (global t)`);
                    }
                    else if (window.r && typeof window.r[addr] !== 'undefined') {
                        value = window.r[addr];
            ...
            const memoryFunctions = Object.keys(h).filter(fn => typeof h[fn] === 'function' &&
    ...
    checkAndResetEmulatorState() {
        ...
        try {
            ...
            const resetFunctions = Object.keys(h).filter(fn => typeof h[fn] === 'function' &&
    ...
    addClickToFocusFunctionality() {
        ...
        this.canvas.addEventListener('click', (event) => {
            ...
            if (!this.emulatorFocused) {
                ...
                (_a = this.canvas) === null || _a === void 0 ? void 0 : _a.focus();
    ...
    forwardKeyEventToEmulator(event) {
    ...
    forwardKeyUpEventToEmulator(event) {
...
exports.VIC20ChipsMachine = VIC20ChipsMachine;
