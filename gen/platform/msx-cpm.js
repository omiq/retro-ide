...
class MSXCPMPlatform extends baseplatform_1.BaseZ80MachinePlatform {
    constructor(mainElement) {
        super(mainElement);
        this.commandHistory = [];
        this.historyIndex = -1;
        this.currentDirectory = "A:";
        this.files = {};
        this.compiledPrograms = {};
        this.setupVirtualFileSystem();
    }
    newMachine() {
        return new msx_1.MSX1();
    }
    getPresets() {
    ...
    setupInputHandling(input, prompt, terminal) {
        const addOutput = (text, color = '#0f0') => {
...
`);
    ...
    async sendBuildStep(buildStep, sourceCode) {
    ...
    updatePrompt() {
        const prompt = this.mainElement.querySelector('.prompt');
        if (prompt) {
            prompt.textContent = `${this.currentDirectory}> `;
        }
    }
    isExecutableFile(cmd, parts) {
    ...
    async executeAsm(parts, addOutput) {
        var _a, _b;
        let filename;
        let sourceCode;
        if (parts.length >= 2) {
...
emu_1.PLATFORMS['msx-cpm'] = MSXCPMPlatform;
// Export the platform class for dynamic loading
exports.default = MSXCPMPlatform;
//# sourceMappingURL=msx-cpm.js.map