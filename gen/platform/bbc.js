...
exports.BBCMicroPlatform = void 0;
const emu_1 = require("../common/emu");
const AcornDFSdisc_1 = require("./AcornDFSdisc");
class BBCMicroPlatform {
    constructor(mainElement) {
        this.machine = null;
        this.currentSSDBlob = null;
    ...
    reset() {
        ...
        const frame = document.getElementById("bbc-iframe");
    ...
    getToolForFilename(filename) {
        ...
        if (lowerFilename.endsWith('.c'))
            return 'cc65';
        if (lowerFilename.endsWith('.asm') || lowerFilename.endsWith('.s'))
            return 'ca65';
        if (lowerFilename.endsWith('.dasm'))
            return 'dasm';
        return 'cc65'; // default
    }
    getDefaultExtension() {
    ...
    loadROM(title, rom) {
        // Validate rom parameter
        if (!rom || !(rom instanceof Uint8Array)) {
            console.error("BBCMicroPlatform loadROM: Invalid rom parameter", rom);
            return;
        }
        console.log("BBCMicroPlatform loadROM called with title:", title, "and", rom.length, "bytes");
        var frame = document.getElementById("bbc-iframe");
        if (frame && frame.contentWindow) {
            ...
            if (isBasicProgram) {
                ...
                try {
                    ...
                    console.error("BBCMicroPlatform: Error decoding BASIC program:", e);
                }
            }
                ...
                const bbc_debug = window.bbc_debug;
                if (bbc_debug && bbc_debug.generateIframeURL) {
                    // Handle async generateIframeURL
                    bbc_debug.generateIframeURL(rom).then((iframeURL) => {
    ...
    async saveFileToServerAndLoad(basicText, frame, modelQuery) {
    ...
    async triggerCompilationAndReload() {
        ...
        const checkWorkerAndCompile = () => {
            ...
            if (worker && worker.postMessage) {
                ...
                const project = (_a = window.IDE) === null || _a === void 0 ? void 0 : _a.getCurrentProject();
    ...
    createSSDForBasicProgram(basicOutput) {
        var _a;
        ...
        const currentFile = ((_a = window.IDE) === null || _a === void 0 ? void 0 : _a.getCurrentMainFilename()) || 'PROGRAM';
        ...
        if (extracted && extracted.data && extracted.data.length > 0) {
            ...
            return {
                extension: '.ssd',
                blob: new Blob([ssdData.buffer], { type: 'application/octet-stream' })
            };
        }
    ...
    extractTokenizedBasicFromEmulator() {
        try {
            ...
            if (page === 0)
                page = 0x1900; // final safety default
    ...
    createProperSSDWithTokenizedBasic(tokenizedBasic, filename, loadExecAddress) {
        ...
        const disc = new AcornDFSdisc_1.AcornDFSdisc();
    ...
    createProperSSD(basicText, filename) {
    ...
    tokenizeBasicToMemoryImage(basicText, loadAddress) {
        ...
        for (const raw of lines) {
            ...
            body.set(tokenBytes, 0);
            body[tokenBytes.length] = 0x00;
    ...
    isBasicProgram(rom) {
        ...
        if (!rom || !(rom instanceof Uint8Array) || rom.length === 0) {
            return false;
        }
        ...
        try {
            ...
            const hasBasicKeywords = basicKeywords.some(keyword => text.toUpperCase().includes(keyword));
            return hasLineNumbers && hasBasicKeywords;
        }
        catch (e) {
            console.error("BBCMicroPlatform: Error decoding ROM in isBasicProgram:", e);
            return false;
        }
    }
...
exports.BBCMicroPlatform = BBCMicroPlatform;
// Register the BBC platform
emu_1.PLATFORMS['bbc'] = BBCMicroPlatform;
emu_1.PLATFORMS['bbc-micro'] = BBCMicroPlatform;
emu_1.PLATFORMS['bbc.b'] = BBCMicroPlatform;
emu_1.PLATFORMS['bbc.model.b'] = BBCMicroPlatform;
